  ...
  return <div className="ec-page ec-delivery-page">
    ...
    {stage && <section className="ec-workspace-grid">
      <div className="ec-work-browser ec-card">
        ...
        <div className="ec-stage-summary"><div><small>Progress</small><strong>{stage.progressPercent.toFixed(0)}%</strong></div><div><small>Package budget</small><strong>{money(stage.budgetAmount,data.currency)}</strong></div><div><small>Actual</small><strong>{money(stage.actualCost,data.currency)}</strong></div><div><small>Blocked</small><strong>{stage.blockedWorkItems}</strong></div></div>
        ...
        {pkg && <div className="ec-work-list"><div className="ec-work-list-head"><div><strong>{pkg.name}</strong><span>{pkg.discipline}</span></div><span>{money(pkg.actualCost,data.currency)} actual · {pkg.totalHours.toFixed(1)}h</span></div>{pkg.workItems.map(item=><button key={item.id} className={`ec-work-row ${item.id===selectedWorkItemId?'active':''}`} onClick={()=>setWorkItemId(item.id)}><div className={`ec-work-state ${statusClass(item.status)}`}/><div className="ec-work-main"><span className="ec-project-code">{item.itemCode}</span><strong>{item.name}</strong><small>{item.responsibleOrganizationName??'Unassigned'} · {item.workType.replaceAll('_',' ')}</small></div><div className="ec-work-metrics"><span>{item.progressPercent.toFixed(0)}%</span><span>{item.documentCount} docs</span><span>{item.totalHours.toFixed(1)}h</span></div><span className={`ec-badge ${statusClass(item.status)}`}>{item.status.replaceAll('_',' ')}</span></button>)}</div>}
      </div>

      <aside className="ec-work-detail">{workItem ? <>
        <div className="ec-card ec-work-detail-card"><div className="ec-detail-head"><div><span className="ec-project-code">{workItem.itemCode}</span><h2>{workItem.name}</h2></div><span className={`ec-badge ${statusClass(workItem.status)}`}>{workItem.status.replaceAll('_',' ')}</span></div><div className="ec-progress ec-progress-lg"><span style={{width:`${Math.min(100,workItem.progressPercent)}%`}}/></div>{workItem.blockedReason&&<div className="ec-blocker"><AlertCircle size={17}/><div><strong>Why this work is blocked</strong><p>{workItem.blockedReason}</p></div></div>}<div className="ec-detail-stats"><div><small>Budget</small><strong>{money(workItem.budgetAmount,data.currency)}</strong></div><div><small>Actual</small><strong>{money(workItem.actualCost,data.currency)}</strong></div><div><small>Variance</small><strong className={variance<0?'ec-text-danger':'ec-text-success'}>{money(variance,data.currency)}</strong></div><div><small>Logged time</small><strong>{workItem.totalHours.toFixed(1)} h</strong></div></div><div className="ec-detail-line"><CalendarDays size={15}/><span>{date(workItem.plannedStart)} → {date(workItem.plannedEnd)}</span></div><div className="ec-detail-line"><Building2 size={15}/><span>{workItem.responsibleOrganizationName??'No responsible organization'}</span></div></div>

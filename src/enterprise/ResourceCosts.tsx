...
export default function ResourceCosts(){
  ...
  useEffect(()=>{if(!projectId)return;Promise.all([
    enterpriseApi.resourceCostSummary(projectId),enterpriseApi.projectResources(projectId),enterpriseApi.actualCosts(projectId,documentFilter||undefined)
  ]).then(([s,r,c])=>{setError('');setSummary(s);setResources(r);setCosts(c)}).catch(e=>setError(String(e)))},[projectId,documentFilter]);
  ...
  return <div className="ec-page">
    ...
    <section className="ec-section"><div className="ec-card-title"><h2>Recent actual cost</h2><span>Latest 200 entries</span></div>
      <div style={{marginBottom:10}}>
        <input className="form-input" style={{maxWidth:340}} placeholder="Filter by document ID — generate a bill for one document"

...
export default function ProjectControls(){
  ...
  useEffect(()=>{if(!projectId)return;setError('');Promise.all([
    enterpriseApi.controlsSummary(projectId), enterpriseApi.projectContracts(projectId), enterpriseApi.currentBudget(projectId), enterpriseApi.forecasts(projectId)
  ]).then(([s,c,b,f])=>{setSummary(s);setContracts(c);setBudget(b);setForecasts(f)}).catch(e=>setError(String(e)))},[projectId]);
  ...
  return <div className="ec-page">
    ...
    {summary&&<>
      <section className="ec-grid ec-kpis">
        <div className="ec-card"><div className="ec-kpi-label">Current budget</div><div className="ec-kpi-value">{money(summary.currentBudget,summary.currency)}</div><div className="ec-kpi-meta">Approved baseline + changes</div></div>
        ...
        <div className="ec-card"><div className="ec-kpi-label">Forecast final cost</div><div className="ec-kpi-value">{money(summary.forecastFinalCost,summary.currency)}</div><div className={`ec-kpi-meta ${summary.forecastVariance<0?'ec-danger':''}`}>{summary.forecastVariance<0?'Projected overrun':'Projected headroom'} {money(Math.abs(summary.forecastVariance),summary.currency)}</div></div>
